...
async function nextBatch(sqlAST, data, dbCall, context, options) {
  ...
  return Promise.all(children.map(childAST => nextBatchChild(childAST, data, dbCall, context, options, children)));
}
async function nextBatchChild(childAST, data, dbCall, context, options, siblings) {
  ...
  const isConflicting = (0, _aliases.hasConflictingSiblings)(childAST, siblings);
  const fieldName = childAST.fieldName;
  const valueKey = isConflicting ? (0, _aliases.getAliasKey)(fieldName, childAST.alias) : fieldName;
  if (childAST.sqlBatch || ((_ref2 = childAST) != null ? (_ref2 = _ref2.junction) != null ? _ref2.sqlBatch : _ref2 : _ref2)) {
    ...
    if (Array.isArray(data)) {
      ...
      const {
        sql,
        shapeDefinition
      } = await (0, _util.compileSqlAST)(childAST, context, {
        ...options,
        batchScope
      });
      ...
      if (childAST.grabMany) {
        for (let obj of data) {
          obj[valueKey] = newData[obj[parentKey]] || (childAST.paginate ? {
            total: 0,
            edges: [],
            pageInfo: {
              startCursor: null,
          ...
          if (isConflicting) obj[fieldName] = _aliases.resolveAliasValue;
        }
      } else {
        let matchedData = [];
        for (let obj of data) {
          ...
          if (ob) {
            obj[valueKey] = Object.assign(obj[valueKey] ?? {}, (0, _arrayToConnection.default)(newData[obj[parentKey]][0], childAST));
      ...
      const nextLevelData = (0, _lodash.chain)(data).filter(obj => obj != null).flatMap(obj => obj[valueKey]).filter(obj => obj != null).value();
    ...
    if (childAST.paginate) {
      ...
      data[valueKey] = (0, _arrayToConnection.default)(targets, childAST);
    ...
    if (isConflicting) data[fieldName] = _aliases.resolveAliasValue;
    if (data) {
